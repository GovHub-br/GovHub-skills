...
Herdado da skill do Gov Hub, onde foi validado na prática (agosto de
2026) num documento de teste de 3 páginas revisado pelo usuário; a
arquitetura não depende da marca. Esta é a técnica correta para gerar PDF a
...
6. **Numere as páginas e o índice no mesmo passo.** O gerador é quem sabe
   ...
   const footer = (pg) => pg.isCover ? '' : `
     ...
     <div class="gh-footer">
       <span class="gh-footer__text"><span class="gh-footer__page">${pg.number}</span>${docShort} &middot; ${projectShort} &middot; Lab Livre &middot; UnB</span>
...
(ex: `logo/lab-livre-white.svg`) já assumem essa cópia local, relativa ao HTML.

**Ícones:** não são copiados — os `src=` nos exemplos apontam direto para a
CDN (`https://cdn.jsdelivr.net/gh/GovHub-br/skills-assets@main/lablivre/icons/...`).
