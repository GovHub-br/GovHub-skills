...
fixa 210×297mm, `@page { margin: 0 }`). Herdado da skill do Gov Hub, onde foi validado com o usuário em
2026-09-17 (simulação do Relatório de Diagnóstico, 7 rodadas de ajuste);
ainda não validado em peça do Lab Livre. O que muda de um documento para outro é só o
...
Fundo **branco**, logo Lab Livre azul (`lab-livre-blue.svg`) no canto
superior esquerdo, título em **Oswald bold uppercase** azul profundo à
esquerda um pouco acima do centro,
...
Paleta da capa: só azul profundo, roxo e rosa claro. Foi testada uma versão com fundo
...
<div class="gh-page gh-cover">
  ...
  <div class="gh-cover__footer">
    ...
    <!-- se for projeto Gov Hub: -->
    <img src="logo/parceiros/govhub-black.svg" alt="Gov Hub">
  </div>
</div>
```

Fundo branco pede parceiros **pretos, na mesma cor**: `unb-dark-outlined.svg`
(UnB sempre `outlined`, sempre presente: o Lab Livre fica dentro dela) e,
se o documento for de projeto Gov Hub, `govhub-black.svg`. **Ordem fixa:
UnB → Gov Hub → Ipea → ministério**, cada um só quando o projeto o inclui,
na versão `positivo` (texto preto). A marca Lab Livre não vai na linha de
parceiros: ela já está no topo da capa. Catálogo, lacunas e regra do defeso eleitoral
...
rodapé de parceiros fica à esquerda; uma linha de dois parceiros (UnB +
Gov Hub a 34px) termina em ~100mm da borda esquerda e o quarto só começa
em ~108mm nessa altura. Se entrar um terceiro parceiro (Ipea, ministério),
...
- **Agosto/2026 (Gov Hub):** fundo roxo sólido `--logo-purple`, moldura arredondada
  branca, logo branca, parceiros brancos centralizados (22px). Substituída.
- **2026-09-17 (Gov Hub, IDV nova):** testadas 4 capas (peach, rosa, magenta, navy);
