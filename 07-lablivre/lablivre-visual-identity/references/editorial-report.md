# Estilo editorial Lab Livre — relatórios longos e frameworks numerados

Herdado do estilo editorial do Gov Hub, projeto do Lab Livre: a fonte é o
livro *Gov Hub: um guia prático para integração e qualificação de dados
públicos* (UnB/Ipea/Lab Livre, 2025) e um ciclo de validação real feito
com o Framework de Briefing do Gov Hub. Aqui vale com a paleta do Lab
Livre; ainda não validado em peça do Lab Livre. Use este padrão quando o pedido for um
...
paleta roxo+laranja de produto — não este arquivo.
...
`references/logo/` tem, por enquanto, 2 assinaturas × 3 cores em SVG
(o resto está pendente, ver `partners.md`):

| Assinatura | Arquivos | O que é |
|---|---|---|
| `lab-livre-*` | `blue`, `black`, `white` | Borboleta + "Lab Livre" lado a lado (viewBox 1514×377). A de uso geral. |
| `borboleta-*` | `blue`, `black`, `white` | Só o ícone (256×256, quadrado). Rodapés e marcas pequenas. |

Cores: `blue` = azul profundo `#080056` (mesmo valor de `--dark-navy`),
para fundo claro e a aplicação padrão enquanto não há SVG roxo; `black`
para fundo claro quando a paleta não for viável; `white` para fundo
escuro (azul profundo, roxo). Regra prática:

| Contexto | Arquivo | Tamanho de referência |
|---|---|---|
| Capa de PDF (fundo branco, ver `print-cover.md`) | `lab-livre-blue.svg` | 12mm de altura |
| Capa/slide com fundo colorido/escuro | `lab-livre-white.svg` | ~64px de altura |
| Rodapé de página de PDF, fundo branco | `borboleta-blue.svg` (ver `print-footer.md`) | 9.5mm de altura |
| Rodapé sobre fundo colorido | `borboleta-white.svg` | ~45px de altura |
| Barra inferior de post sobre rosa claro | `lab-livre-blue.svg` | 44px de altura |

A assinatura do Lab Livre é mais comprida que a do Gov Hub (proporção 4:1
contra 4,3:1) e a borboleta é quadrada, então na mesma `height` os
arquivos saem com larguras diferentes das da skill do Gov Hub; mantenha a
altura e deixe a largura em `auto`. Se um dia a logo for reexportada e
vier com espaçamento embutido no SVG, o sintoma é a logo parecer
desalinhada/mais para dentro do que o texto ao lado; ajuste o `viewBox`
para o bounding box real do desenho, não tente compensar só no CSS.

```html
<img src="logo/lab-livre-white.svg" alt="Lab Livre" style="height:64px; width:auto;">
```

Não escreva "Lab Livre" como texto ao lado da logo (ex. "· Lab Livre") a menos
...
- [`print-cover.md`](print-cover.md) — capa: fundo branco, logo azul no
  alto à esquerda, título Oswald uppercase azul profundo, três formas
  sangrando (quarto roxo, pílula rosa claro, anel azul profundo),
  parceiros pretos no rodapé da capa.
...
não cobre isso — use a skill separada `lablivre-icon-creation`, dedicada à
...
variante — inclusive roxo `#7023E8`, magenta e branco. O fundo pintado do
...
- Qualquer contexto onde a marca precisa ser reconhecida instantaneamente
  como "produto Lab Livre" (não como "publicação/relatório Lab Livre") → prefira
