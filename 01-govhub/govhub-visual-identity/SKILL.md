...
- [`references/icons/`](references/icons/) — biblioteca completa de ícones de produto GovHub, 332 nomes × variante `Default` (duotone roxo+laranja, para fundo branco), `orange` e `purple` (fundo sólido colorido), todos em SVG (996 arquivos). Para capas/callouts de relatório use a variante `Default`. Escolha o ícone **pelo nome** (são descritivos, ex.: `document-check`, `shield-check`, `chart-bar`) — a biblioteca é grande demais pra um mapeamento fixo; há um subconjunto curado só para os callouts do Framework de Briefing em `editorial-report.md` seção 5. **Nunca** use os ícones sobre fundo diferente de branco/roxo GovHub/laranja GovHub — ver regra completa e a borda do badge em `editorial-report.md` seção 5.
