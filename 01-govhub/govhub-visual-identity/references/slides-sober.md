# Slides Gov Hub, variante sóbria: código exato validado

Variante **branca** dos slides, validada com o usuário em 2026-09-18
(4 rodadas de ajuste). É a linguagem do relatório em PDF
...
|---|---|---|---|---|
| Capa | 1 por deck | quarto roxo + pílula pêssego + anel navy (as da capa do PDF) | logo navy no alto | parceiros pretos |
...
| Encerramento | 1 por deck | as três da capa | logo navy no alto | contato + parceiros pretos |
...
:root {
  ...
  --text-muted:     #666666;
  --border-soft:    #E9DFFF;
  --section-color:  #0A005A;   /* sempre navy no PDF (print-table.md) */
...
- **Rodapé** é só o **número da página** (30px, navy, bold) à esquerda e o
  símbolo `icone-none-navy.svg` (52px) à direita. **Sem barra e sem nome
  do deck**: a primeira versão tinha a barra e o texto "deck · projeto ·
  Gov Hub · Lab Livre - UnB" do rodapé de PDF, e o usuário pediu para
...
- **Citação** com filete roxo de 6px à esquerda, texto navy 46px, autor
  em `--text-muted`.
- **Encerramento** sem "Obrigado": logo navy no alto, área livre no meio,
  contato (`gov-hub.io` em negrito navy + e-mail) no canto inferior
  esquerdo a `bottom: 210px`, parceiros abaixo. O usuário removeu o
  "Obrigado" e o GitHub de propósito, para deixar mais área livre.
- **Parceiros** na capa e no encerramento: `lab-livre-black.svg` +
  `unb-dark-outlined.svg` a 64px, nessa ordem (regra de `partners.md`).
...
<section class="gh-slide gh-cover">
  ...
  <svg class="gh-shape" viewBox="0 0 100 100" aria-hidden="true"
    ...
    <path fill="#613EFF" d="M100 100H0a100 100 0 0 1 100-100z"/>
  </svg>
  ...
  <svg class="gh-shape" viewBox="0 0 100 100" aria-hidden="true"
    ...
    <path fill="#0A005A" fill-rule="evenodd"
  ...
  <div class="gh-body">
    <div class="gh-cols">
      <div>
        ...
        <p>Nesse cenário, surge a parceria entre o órgão e o Lab Livre da UnB para implantar uma solução de software livre baseada no ecossistema Gov Hub.</p>
      </div>
      <div>
        <h3>Objetivo</h3>
...
<section class="gh-slide gh-closing">
  ...
  <div class="gh-closing__block">
    <p class="gh-closing__lead"><strong>gov-hub.io</strong><br>caguiar@unb.br</p>
  </div>
  <div class="gh-cover__footer">
    <img src="logo/parceiros/lab-livre-black.svg" alt="Lab Livre">
    <img src="logo/parceiros/unb-dark-outlined.svg" alt="UnB">
...
1. **`currentColor` e `transform` em SVG inline não funcionam.** O
   ...
   Nos slides o `fill` é literal (`#613EFF`, `#0A005A`, `#FFE7E1`) e o
